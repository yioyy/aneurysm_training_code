...
import nnxnet
from nnxnet.imageio.natural_image_reader_writer import NaturalImage2DIO
from nnxnet.imageio.nibabel_reader_writer import NibabelIO, NibabelIOWithReorient
from nnxnet.imageio.simpleitk_reader_writer import SimpleITKIO
from nnxnet.imageio.tif_reader_writer import Tiff3DIO
...
def determine_reader_writer_from_dataset_json(dataset_json_content: dict, example_file: str = None,
    ...
    if 'overwrite_image_reader_writer' in dataset_json_content.keys() and \
        ...
        # trying to find that class in the nnxnet.imageio module
        try:
...
def recursive_find_reader_writer_by_name(rw_class_name: str) -> Type[BaseReaderWriter]:
    ret = recursive_find_python_class(join(nnxnet.__path__[0], "imageio"), rw_class_name, 'nnxnet.imageio')
    if ret is None:
        raise RuntimeError("Unable to find reader writer class '%s'. Please make sure this class is located in the "
                           "nnxnet.imageio module." % rw_class_name)
    else:
        return ret
