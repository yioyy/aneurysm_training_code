    ...
    def read_images(self, image_fnames: Union[List[str], Tuple[str, ...]]) -> Tuple[np.ndarray, dict]:
        ...
        for f in image_fnames:
            ...
            if npy_image.ndim == 2:
                ...
                spacings_for_nnxnet.append((max_spacing * 999, *list(spacings[-1])[::-1]))
            elif npy_image.ndim == 3:
                # 3d, as in original nnxnet
                npy_image = npy_image[None]
                spacings_for_nnxnet.append(list(spacings[-1])[::-1])
            elif npy_image.ndim == 4:
                # 4d, multiple modalities in one file
                spacings_for_nnxnet.append(list(spacings[-1])[::-1][1:])
                pass
            else:
                raise RuntimeError(f"Unexpected number of dimensions: {npy_image.ndim} in file {f}")

            images.append(npy_image)
            spacings_for_nnxnet[-1] = list(np.abs(spacings_for_nnxnet[-1]))
        ...
        if not self._check_all_same(origins):
            ...
            print('Image files:')
            print(image_fnames)
            print('It is up to you to decide whether that\'s a problem. You should run nnXNet_plot_overlay_pngs to verify '
        ...
        if not self._check_all_same(directions):
            ...
            print('It is up to you to decide whether that\'s a problem. You should run nnXNet_plot_overlay_pngs to verify '
                  'that segmentations and data overlap.')
        if not self._check_all_same(spacings_for_nnxnet):
            print('ERROR! Not all input images have the same spacing_for_nnxnet! (This should not happen and must be a '
                  'bug. Please report!')
            print('spacings_for_nnxnet:')
            print(spacings_for_nnxnet)
            print('Image files:')
            print(image_fnames)
            raise RuntimeError()
        ...
        dict = {
            'sitk_stuff': {
                # this saves the sitk geometry information. This part is NOT used by nnX-Net!
                'spacing': spacings[0],
                'origin': origins[0],
