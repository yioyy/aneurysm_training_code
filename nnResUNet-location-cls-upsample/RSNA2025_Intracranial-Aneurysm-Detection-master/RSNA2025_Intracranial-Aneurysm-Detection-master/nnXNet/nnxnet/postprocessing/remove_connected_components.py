...
from nnxnet.evaluation.evaluate_predictions import region_or_label_to_mask, compute_metrics_on_folder, \
    load_summary_json, label_or_region_to_key
from nnxnet.imageio.base_reader_writer import BaseReaderWriter
from nnxnet.paths import nnXNet_raw
from nnxnet.utilities.file_path_utilities import folds_tuple_to_string
...
def determine_postprocessing(folder_predictions: str,
    ...
    """
    Determines nnXNet postprocessing. Its output is a postprocessing.pkl file in folder_predictions which can be
    ...
    if plans_file_or_dict is None:
        ...
        if not isfile(expected_plans_file):
            raise RuntimeError(f"Expected plans file missing: {expected_plans_file}. The plans files should have been "
                               f"created while running nnXNet_predict. Sadge.")
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as pool:
        ...
        # in the old nnX-Net we could just apply all-but-largest component removal to all classes at the same time and
    ...
    if plans_file_or_dict is None:
        ...
        if not isfile(expected_plans_file):
            raise RuntimeError(f"Expected plans file missing: {expected_plans_file}. The plans file should have been "
                               f"created while running nnXNet_predict. Sadge. If the folder you want to apply "
    ...
    if dataset_json_file_or_dict is None:
        ...
        if not isfile(expected_dataset_json_file):
            raise RuntimeError(
                ...
                f"copied while running nnXNet_predict/nnXNet_ensemble. Sadge.")
        dataset_json_file_or_dict = load_json(expected_dataset_json_file)
...
if __name__ == '__main__':
    trained_model_folder = '/yinghepool/shipengcheng/Dataset/nnUNet/nnUNet_results/Dataset004_Hippocampus/nnXNetTrainer__nnXNetPlans__3d_fullres'
    labelstr = join(nnXNet_raw, 'Dataset004_Hippocampus', 'labelsTr')
