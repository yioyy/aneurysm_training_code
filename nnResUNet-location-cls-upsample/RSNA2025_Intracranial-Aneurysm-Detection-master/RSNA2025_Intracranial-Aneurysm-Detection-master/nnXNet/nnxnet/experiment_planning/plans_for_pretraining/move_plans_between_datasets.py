...
from nnxnet.utilities.file_path_utilities import maybe_convert_to_dataset_name
...
def move_plans_between_datasets(
    ...
    source_folder = join(nnXNet_preprocessed, source_dataset_name)
    assert isdir(source_folder), f"Cannot move plans because preprocessed directory of source dataset is missing. " \
                                 f"Run nnXNet_plan_and_preprocess for source dataset first!"
    ...
    target_raw_data_dir = join(nnXNet_raw, target_dataset_name)
    ...
    save_json(source_plans, join(nnXNet_preprocessed, target_dataset_name, target_plans_identifier + '.json'),
              sort_keys=False)
...
def entry_point_move_plans_between_datasets():
    ...
    parser.add_argument('-sp', type=str, required=True,
                        help='Source plans identifier. If your plans are named "nnXNetPlans.json" then the '
                             'identifier would be nnXNetPlans')
    parser.add_argument('-tp', type=str, required=False, default=None,
                        help='Target plans identifier. Default is None meaning the source plans identifier will '
                             'be kept. Not recommended if the source plans identifier is a default nnX-Net identifier '
                             'such as nnXNetPlans!!!')
...
if __name__ == '__main__':
    move_plans_between_datasets(2, 4, 'nnXNetPlans', 'nnXNetPlansFrom2')
