...
from nnxnet.configuration import ANISO_THRESHOLD
from nnxnet.experiment_planning.experiment_planners.network_topology import get_pool_and_conv_props
from nnxnet.imageio.reader_writer_registry import determine_reader_writer_from_dataset_json
from nnxnet.paths import nnXNet_raw, nnXNet_preprocessed
from nnxnet.preprocessing.normalization.map_channel_name_to_normalization import get_normalization_scheme
from nnxnet.preprocessing.resampling.default_resampling import resample_data_or_seg_to_shape, compute_new_shape
...
from nnxnet.utilities.utils import get_filenames_of_train_images_and_targets


class ExperimentPlanner(object):
    def __init__(self, dataset_name_or_id: Union[str, int],
                 gpu_memory_target_in_gb: float = 8,
                 preprocessor_name: str = 'DefaultPreprocessor', plans_name: str = 'nnXNetPlans',
        ...
        self.raw_dataset_folder = join(nnXNet_raw, self.dataset_name)
        preprocessed_folder = join(nnXNet_preprocessed, self.dataset_name)
        ...
        if not isfile(join(preprocessed_folder, 'dataset_fingerprint.json')):
            raise RuntimeError('Fingerprint missing for this dataset. Please run nnXNet_extract_dataset_fingerprint')
        ...
        # the following two numbers are really arbitrary and were set to reproduce nnX-Net v1's configurations as
        # much as possible
    ...
    def get_plans_for_configuration(self,
        ...
        # this is different from how nnX-Net v1 does it!
    ...
    def plan_experiment(self):
        ...
        shutil.copy(join(self.raw_dataset_folder, 'dataset.json'),
                    join(nnXNet_preprocessed, self.dataset_name, 'dataset.json'))
    ...
    def save_plans(self, plans):
        ...
        plans_file = join(nnXNet_preprocessed, self.dataset_name, self.plans_identifier + '.json')
        ...
        maybe_mkdir_p(join(nnXNet_preprocessed, self.dataset_name))
        save_json(plans, plans_file, sort_keys=False)
        print(f"Plans were saved to {join(nnXNet_preprocessed, self.dataset_name, self.plans_identifier + '.json')}")
