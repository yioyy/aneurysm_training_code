...
from nnxnet.experiment_planning.dataset_fingerprint.fingerprint_extractor import DatasetFingerprintExtractor
from nnxnet.experiment_planning.experiment_planners.default_experiment_planner import ExperimentPlanner
from nnxnet.experiment_planning.verify_dataset_integrity import verify_dataset_integrity
from nnxnet.paths import nnXNet_raw, nnXNet_preprocessed
from nnxnet.utilities.dataset_name_id_conversion import convert_id_to_dataset_name
...
def extract_fingerprint_dataset(dataset_id: int,
    ...
    if check_dataset_integrity:
        verify_dataset_integrity(join(nnXNet_raw, dataset_name), num_processes)
...
def extract_fingerprints(dataset_ids: List[int], fingerprint_extractor_class_name: str = 'DatasetFingerprintExtractor',
                         ...
                         clean: bool = True, verbose: bool = True):
    """
    clean = False will not actually run this. This is just a switch for use with nnXNet_plan_and_preprocess where
    we don't want to rerun fingerprint extraction every time.
    """
    fingerprint_extractor_class = recursive_find_python_class(join(nnxnet.__path__[0], "experiment_planning"),
...
def plan_experiments(dataset_ids: List[int], experiment_planner_class_name: str = 'ExperimentPlanner',
    ...
    if experiment_planner_class_name == 'ExperimentPlanner':
        print("\n############################\n"
              "INFO: You are using the old nnX-Net default planner. We have updated our recommendations. "
...
def preprocess_dataset(dataset_id: int,
    ...
    # copy the gt to a folder in the nnXNet_preprocessed so that we can do validation even if the raw data is no
    # longer there (useful for compute cluster where only the preprocessed data is available)
    from distutils.file_util import copy_file
    maybe_mkdir_p(join(nnXNet_preprocessed, dataset_name, 'gt_segmentations'))
    dataset_json = load_json(join(nnXNet_raw, dataset_name, 'dataset.json'))
    ...
    for k in dataset:
        copy_file(dataset[k]['label'],
                  join(nnXNet_preprocessed, dataset_name, 'gt_segmentations', k + dataset_json['file_ending']),
                  update=True)
