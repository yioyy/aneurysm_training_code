...
def ensemble_folders(list_of_input_folders: List[str],
    ...
    and/or dataset.json in it. These are usually copied into those folders by nnX-Net during prediction.
        ...
        for f in folds:
            ...
            if len(files_here) == 0:
                raise RuntimeError(f"No .npz files found in folder {join(tr, f'fold_{f}', 'validation')}. Rerun your "
                                   f"validation with the --npz flag. Use nnXNet_train [...] --val --npz.")
