from nnxnet.training.loss.compound_losses import DC_and_topk_loss
...
import numpy as np
from nnxnet.training.loss.robust_ce_loss import TopKLoss


class nnXNetTrainerTopk10Loss(nnXNetTrainer):
...
class nnXNetTrainerTopk10LossLS01(nnXNetTrainer):
    def _build_loss(self):
        assert not self.label_manager.has_regions, "regions not supported by this trainer"
        loss = TopKLoss(
...
class nnXNetTrainerDiceTopK10Loss(nnXNetTrainer):
    def _build_loss(self):
        assert not self.label_manager.has_regions, "regions not supported by this trainer"
