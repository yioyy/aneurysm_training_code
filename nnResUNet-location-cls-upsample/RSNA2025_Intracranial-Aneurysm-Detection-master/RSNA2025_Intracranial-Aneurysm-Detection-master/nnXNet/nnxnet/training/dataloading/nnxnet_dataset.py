...
from nnxnet.training.dataloading.utils import get_case_identifiers


class nnXNetDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        IMPORTANT! THIS CLASS ITSELF IS READ-ONLY. YOU CANNOT ADD KEY:VALUE PAIRS WITH nnXNetDataset[key] = value
        USE THIS INSTEAD:
        nnXNetDataset.dataset[key] = value
        ...
        self.keep_files_open = ('nnXNet_keep_files_open' in os.environ.keys()) and \
                               (os.environ['nnXNet_keep_files_open'].lower() in ('true', '1', 't'))
        # print(f'nnXNetDataset.keep_files_open: {self.keep_files_open}')

    def __getitem__(self, key):
        ret = {**self.dataset[key]}
        if 'properties' not in ret.keys():
            # print("Loading properties for key ", key)
...
if __name__ == '__main__':
    ...
    folder = '/yinghepool/shipengcheng/Dataset/nnUNet/nnXNet_preprocessed/Dataset003_Liver/3d_lowres'
    ds = nnXNetDataset(folder, num_images_properties_loading_threshold=0) # this should not load the properties!
    ...
    # this should have the properties
    ds = nnXNetDataset(folder, num_images_properties_loading_threshold=1000)
    ...
    ds = nnXNetDataset(folder, num_images_properties_loading_threshold=0)
    # now rename the properties file so that it does not exist anymore
    shutil.move(join(folder, 'liver_0.pkl'), join(folder, 'liver_XXX.pkl'))
