The nnX-Net inference is now much more dynamic than before, allowing you to more seamlessly integrate nnX-Net into 
your existing workflows.
This readme will give you a quick rundown of your options. This is not a complete guide. Look into the code to learn 
all the details!

# Preface
In terms of speed, the most efficient inference strategy is the one done by the nnX-Net defaults! Images are read on 
...
# nnXNetPredictor
The new nnXNetPredictor class encapsulates the inferencing code and makes it simple to switch between modes. Your 
code can hold a nnXNetPredictor instance and perform prediction on the fly. Previously this was not possible and each 
new prediction request resulted in reloading the parameters and reinstantiating the network architecture. Not ideal.

The nnXNetPredictor must be ininitialized manually! You will want to use the 
...
## Recommended nnX-Net default: predict from source files
...
```python
    ...
    # instantiate the nnXNetPredictor
...
```python
    ...
    indir = join(nnXNet_raw, 'Dataset003_Liver/imagesTs')
    outdir = join(nnXNet_raw, 'Dataset003_Liver/imagesTs_predlowres')
...
```python
    ...
    img, props = SimpleITKIO().read_images([join(nnXNet_raw, 'Dataset003_Liver/imagesTs/liver_147_0000.nii.gz')])
    img2, props2 = SimpleITKIO().read_images([join(nnXNet_raw, 'Dataset003_Liver/imagesTs/liver_146_0000.nii.gz')])
    img3, props3 = SimpleITKIO().read_images([join(nnXNet_raw, 'Dataset003_Liver/imagesTs/liver_145_0000.nii.gz')])
    img4, props4 = SimpleITKIO().read_images([join(nnXNet_raw, 'Dataset003_Liver/imagesTs/liver_144_0000.nii.gz')])
...
cons:
- slows as heck, yo
- never the right choice unless you can only give a single image at a time to nnX-Net
...
```python
    ...
    img, props = NibabelIO().read_images([join(nnXNet_raw, 'Dataset003_Liver/imagesTr/liver_63_0000.nii.gz')])
    ret = predictor.predict_single_npy_array(img, props, None, None, False)

    # The following IS NOT RECOMMENDED. Use nnxnet.imageio!
    # nibabel, we need to transpose axes and spacing to match the training axes ordering for the nnX-Net default:
