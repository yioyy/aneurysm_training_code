...
from nnxnet.paths import nnXNet_preprocessed, nnXNet_raw
from nnxnet.preprocessing.cropping.cropping import crop_to_nonzero
from nnxnet.preprocessing.resampling.default_resampling import compute_new_shape
...
from nnxnet.utilities.utils import get_identifiers_from_splitted_dataset_folder, \
...
class DefaultPreprocessor(object):
    ...
    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str,
        ...
        output_npz = output_filename_truncated + '.npz'
        output_pkl = output_filename_truncated + '.pkl'

        if not (os.path.exists(output_npz) and os.path.exists(output_pkl)):
            data, seg, properties = self.run_case(image_files, seg_file, plans_manager, configuration_manager, dataset_json)
            # print('dtypes', data.dtype, seg.dtype)
            np.savez_compressed(output_npz, data=data, seg=seg)
            write_pickle(properties, output_pkl)
    ...
    def _normalize(self, data: np.ndarray, seg: np.ndarray, configuration_manager: ConfigurationManager,
        ...
        for c in range(data.shape[0]):
            ...
            normalizer_class = recursive_find_python_class(join(nnxnet.__path__[0], "preprocessing", "normalization"),
                                                           scheme,
                                                           'nnxnet.preprocessing.normalization')
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        assert isdir(join(nnXNet_raw, dataset_name)), "The requested dataset could not be found in nnXNet_raw"

        plans_file = join(nnXNet_preprocessed, dataset_name, plans_identifier + '.json')
        assert isfile(plans_file), "Expected plans file (%s) not found. Run corresponding nnXNet_plan_experiment " \
                                   "first." % plans_file
        ...
        dataset_json_file = join(nnXNet_preprocessed, dataset_name, 'dataset.json')
        dataset_json = load_json(dataset_json_file)

        output_directory = join(nnXNet_preprocessed, dataset_name, configuration_manager.data_identifier)
        ...
        dataset = get_filenames_of_train_images_and_targets(join(nnXNet_raw, dataset_name), dataset_json)
...
def example_test_case_preprocessing():
    ...
    plans_file = '/home/isensee/drives/gpu_data/nnXNet_preprocessed/Dataset219_AMOS2022_postChallenge_task2/nnXNetPlans.json'
    dataset_json_file = '/home/isensee/drives/gpu_data/nnXNet_preprocessed/Dataset219_AMOS2022_postChallenge_task2/dataset.json'
    input_images = ['/home/isensee/drives/e132-rohdaten/nnXNet/Dataset219_AMOS2022_postChallenge_task2/imagesTr/amos_0600_0000.nii.gz', ]  # if you only have one channel, you still need a list: ['case000_0000.nii.gz']
    ...
    # voila. Now plug data into your prediction function of choice. We of course recommend nnX-Net's default (TODO)
    return data
