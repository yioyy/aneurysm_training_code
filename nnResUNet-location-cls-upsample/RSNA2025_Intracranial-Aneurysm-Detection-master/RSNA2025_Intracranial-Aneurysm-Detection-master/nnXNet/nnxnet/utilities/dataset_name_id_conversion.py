...
def find_candidate_datasets(dataset_id: int):
    ...
    if nnXNet_preprocessed is not None and isdir(nnXNet_preprocessed):
        candidates_preprocessed = subdirs(nnXNet_preprocessed, prefix=startswith, join=False)
    else:
        candidates_preprocessed = []

    if nnXNet_raw is not None and isdir(nnXNet_raw):
        candidates_raw = subdirs(nnXNet_raw, prefix=startswith, join=False)
    else:
        candidates_raw = []

    candidates_trained_models = []
    if nnXNet_results is not None and isdir(nnXNet_results):
        candidates_trained_models += subdirs(nnXNet_results, prefix=startswith, join=False)
...
def convert_id_to_dataset_name(dataset_id: int):
    ...
    if len(unique_candidates) > 1:
        raise RuntimeError("More than one dataset name found for dataset id %d. Please correct that. (I looked in the "
                           "following folders:\n%s\n%s\n%s" % (dataset_id, nnXNet_raw, nnXNet_preprocessed, nnXNet_results))
    if len(unique_candidates) == 0:
        raise RuntimeError(f"Could not find a dataset with the ID {dataset_id}. Make sure the requested dataset ID "
                           f"exists and that nnX-Net knows where raw and preprocessed data are located "
                           f"(see Documentation - Installation). Here are your currently defined folders:\n"
                           f"nnXNet_preprocessed={os.environ.get('nnXNet_preprocessed') if os.environ.get('nnXNet_preprocessed') is not None else 'None'}\n"
                           f"nnXNet_results={os.environ.get('nnXNet_results') if os.environ.get('nnXNet_results') is not None else 'None'}\n"
                           f"nnXNet_raw={os.environ.get('nnXNet_raw') if os.environ.get('nnXNet_raw') is not None else 'None'}\n"
