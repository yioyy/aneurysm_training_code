...
from nnxnet.utilities.file_path_utilities import *
...
def export_pretrained_model(dataset_name_or_id: Union[int, str], output_file: str,
    ...
    with(zipfile.ZipFile(output_file, 'w', zipfile.ZIP_DEFLATED)) as zipf:
        for c in configurations:
            ...
            for fold_folder in expected_fold_folder:
                ...
                if isfile(source_file):
                    zipf.write(source_file, os.path.relpath(source_file, nnXNet_results))
                ...
                if export_crossval_predictions:
                    ...
                    for f in files:
                        zipf.write(join(source_folder, f), os.path.relpath(join(source_folder, f), nnXNet_results))
            ...
            if isdir(source_folder):
                ...
                for s in source_files:
                    if isfile(s):
                        zipf.write(s, os.path.relpath(s, nnXNet_results))
            # plans
        ...
        ensemble_dir = join(nnXNet_results, dataset_name, 'ensembles')
        ...
        for ens in subd:
            ...
            if ok:
                ...
                for s in source_files:
                    zipf.write(s, os.path.relpath(s, nnXNet_results))
        inference_information_file = join(nnXNet_results, dataset_name, 'inference_information.json')
        if isfile(inference_information_file):
            zipf.write(inference_information_file, os.path.relpath(inference_information_file, nnXNet_results))
        inference_information_txt_file = join(nnXNet_results, dataset_name, 'inference_information.txt')
        if isfile(inference_information_txt_file):
            zipf.write(inference_information_txt_file, os.path.relpath(inference_information_txt_file, nnXNet_results))
    print('Done')
