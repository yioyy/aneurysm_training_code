...
def _extract_seg(output):
    """If network returns (seg_output, cls_output) tuple, extract only seg_output."""
...
def maybe_mirror_and_predict(network: nn.Module, x: torch.Tensor, mirror_axes: Tuple[int, ...] = None) \
        -> torch.Tensor:
    prediction = _extract_seg(network(x))
    ...
    if mirror_axes is not None:
        ...
        if 0 in mirror_axes:
            prediction += torch.flip(_extract_seg(network(torch.flip(x, (2,)))), (2,))
        if 1 in mirror_axes:
            prediction += torch.flip(_extract_seg(network(torch.flip(x, (3,)))), (3,))
        if 2 in mirror_axes:
            prediction += torch.flip(_extract_seg(network(torch.flip(x, (4,)))), (4,))
        if 0 in mirror_axes and 1 in mirror_axes:
            prediction += torch.flip(_extract_seg(network(torch.flip(x, (2, 3)))), (2, 3))
        if 0 in mirror_axes and 2 in mirror_axes:
            prediction += torch.flip(_extract_seg(network(torch.flip(x, (2, 4)))), (2, 4))
        if 1 in mirror_axes and 2 in mirror_axes:
            prediction += torch.flip(_extract_seg(network(torch.flip(x, (3, 4)))), (3, 4))
        if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
            prediction += torch.flip(_extract_seg(network(torch.flip(x, (2, 3, 4)))), (2, 3, 4))
