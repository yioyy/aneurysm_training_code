...
class NewSoftDiceLoss(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
                 ddp: bool = True, clip_tp: float = None):
        """
        """
        super(NewSoftDiceLoss, self).__init__()
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        #print('before dc.shape:', dc.shape)
        ...
        if not self.do_bg:
            if self.batch_dice:
                ...
                dc = dc[:, 1:]
        #print('after dc.shape:', dc.shape)
        dc = dc.mean()

        #print('self.apply_nonlin:', self.apply_nonlin)，確定這裡輸出會做softmax
...
class MemoryEfficientSoftDiceLoss(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
                 ddp: bool = True, channel_weights: torch.Tensor = None):
        """
        saves 1.6 GB on Dataset017 3d_lowres
        channel_weights: optional per-channel weights for region-based training, shape (num_channels,)
        """
...
class MemoryEfficientNewSoftDiceLoss(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
                 ddp: bool = True, channel_weights: torch.Tensor = None):
        """
        saves 1.6 GB on Dataset017 3d_lowres
        channel_weights: optional per-channel weights for region-based training, shape (num_channels,)
        """
        super(MemoryEfficientNewSoftDiceLoss, self).__init__()
...
class MemoryEfficientLogDiceLoss(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
        ...
        saves 1.6 GB on Dataset017 3d_lowres
        """
        super(MemoryEfficientLogDiceLoss, self).__init__()
