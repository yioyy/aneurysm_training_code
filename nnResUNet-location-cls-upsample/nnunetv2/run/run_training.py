...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    if enable_ema:
        print(f'CLI: ENABLE_EMA = True (啟用 EMA model, decay={ema_decay})')
    ...
    # CLI --region_loss_weights 覆蓋 dataset.json 中的 region_loss_weights
    if region_loss_weights is not None:
        dataset_json['region_loss_weights'] = region_loss_weights
        print(f'CLI override region_loss_weights: {region_loss_weights}')
...
def run_training_entry():
    ...
    # Region-based multi-label loss weighting
    parser.add_argument('--region_loss_weights', type=str, default=None, required=False,
                        help='[OPTIONAL] Per-channel loss weights for region-based (multi-label) training. '
    ...
    parser.add_argument('--enable_deep_supervision_logging', action='store_true', required=False,
                        help='[OPTIONAL] Enable per-level deep supervision logging (ce_loss, dice_loss, '
                             'individual dice per level, fake_dcl). Disabled by default to save ~2x loss computation.')

    # EMA (Exponential Moving Average) model
    parser.add_argument('--enable_ema', action='store_true', required=False,
                        help='[OPTIONAL] Enable EMA model. Maintains a smoothed copy of network weights, '
                             'runs separate EMA validation each epoch, and saves checkpoint_best_ema.pth.')
    parser.add_argument('--ema_decay', type=float, default=0.999, required=False,
                        help='[OPTIONAL] EMA decay factor. Higher = smoother/slower update. Default: 0.999')

    # 分類頭前景 label 設定
    parser.add_argument('--cls_foreground_labels', type=str, default=None, required=False,
                        help='[OPTIONAL] Classifier head foreground labels. Comma-separated ints, e.g. "1" or "1,2". '
                             'Only these seg label values count as positive for the classifier head. '
    ...
    # 解析 region_loss_weights: "5,1,1,1,1" -> [5.0, 1.0, 1.0, 1.0, 1.0]
